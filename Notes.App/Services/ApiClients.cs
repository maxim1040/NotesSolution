...
public class ApiClient
{
    ...
    public Task<List<NoteDto>?> GetNotesAsync() =>
        _http.GetFromJsonAsync<List<NoteDto>>($"{Constants.ApiBase}/api/notes");

    public async Task<NoteDto?> CreateAsync(NoteModel n)
    {
        var res = await _http.PostAsJsonAsync($"{Constants.ApiBase}/api/notes",
            new NoteCreateDto(n.Title, n.Content));
    ...
    public Task<HttpResponseMessage> UpdateAsync(NoteModel n) =>
        _http.PutAsJsonAsync($"{Constants.ApiBase}/api/notes/{n.Id}",
            new NoteUpdateDto(n.Title, n.Content, n.UpdatedAtUtc));

    public Task<HttpResponseMessage> DeleteAsync(Guid id) =>
        _http.DeleteAsync($"{Constants.ApiBase}/api/notes/{id}");
}
